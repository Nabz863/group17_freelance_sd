...
export default function PostJobForm({ embed = false }) {
  ...
  const [formData, setFormData]       = useState({ title:'', description:'', requirements:'', budget:'', deadline:'' });
  const [milestones, setMilestones]   = useState([{ title:'', dueDate:'', amount:'' }]);
  const [submitting, setSubmitting]   = useState(false);
  const [submitted,  setSubmitted]    = useState(false);
  const [error,      setError]        = useState(null);
  const [template,   setTemplate]     = useState([]);

  // — load contract template once —
  useEffect(() => {
    (async () => {
      try {
    ...
    try {
      ...
      if (msToInsert.length) {
        const { error: mErr } = await supabase
          .from('milestones')
          .insert(msToInsert);
        if (mErr) console.warn('Milestones insert failed:', mErr);
      }

      // 3) immediately create default contract
      try {
        await createContract({
          projectId: project.id,
          title:     formData.title,
          freelancerId: '',           // assigned later via dashboard
          contractSections: template  // if empty, backend will fallback to its default
        });
      } catch (cErr) {
        console.warn('Contract creation skipped:', cErr);
      }
  ...
  if (submitted) {
    return (
      <section className="text-center">
        ...
        {!embed && (
          <button className="primary-btn mt-4" onClick={() => navigate('/client')}>
            Back to Dashboard
          </button>
        )}
      </section>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
      {!embed && <>
        ...
        <p className="text-gray-400 mb-6">Describe your project and attract the right freelancers</p>
      </>}
      ...
      <div className="border border-gray-700 rounded-lg p-4 mb-6">
        ...
        {milestones.map((m, i) => (
          <div key={i} className="mb-4 p-4 border border-gray-700 rounded bg-[#1a1a1a]">
            <div className="flex justify-between items-center mb-2">
              ...
              <button type="button" onClick={() => handleRemoveMilestone(i)}
        ...
        <button type="button" onClick={handleAddMilestone}
                className="mt-2 px-4 py-1 border border-gray-600 rounded text-gray-300 hover:bg-gray-800">
          + Add Milestone
        </button>
      </div>
      ...
      <div className="flex justify-end space-x-4">
        {!embed && (
          <button type="button" onClick={() => navigate('/client')}
                  className="px-6 py-2 border border-gray-600 rounded-md text-gray-300 hover:bg-gray-800">
            Cancel
          </button>
        )}
        <button type="submit" disabled={submitting}
                className="px-6 py-2 bg-[#1abc9c] rounded-md text-white hover:bg-[#16a085] disabled:opacity-50">
