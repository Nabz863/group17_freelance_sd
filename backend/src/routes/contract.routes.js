...
} = require("../middleware/contractValidation.middleware");
const {
  getContractTemplate,
  createContract,
  getContractById,
...
router.get("/template", jwtCheck, checkRole(["client"]), async (req, res) => {
  try {
...
// Get contract by ID
router.get("/:contractId", jwtCheck, async (req, res) => {
  try {
...
router.patch("/:contractId/status", jwtCheck, async (req, res) => {
  try {
    const { contractId } = req.params;
    ...
    const isFreelancer =
      req.user.role === "freelancer" && contract.freelancerId === req.auth.sub;
    const isClient =
      req.user.role === "client" && contract.clientId === req.auth.sub;
...
router.get("/:contractId/document", jwtCheck, async (req, res) => {
  try {
    const { contractId } = req.params;
