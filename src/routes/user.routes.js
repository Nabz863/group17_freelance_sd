...
router.patch('/:userId/status', verifyToken, checkRole(['admin']), (req, res) => {
  ...
  // need to include sending a notification to user about status update
...
router.get('/profile', verifyToken, (req, res) => {
  ...
  res.status(200).json({
    ...
    user: {
      ...
      name: user.name,
      role: user.role,
      // Include other profile details based on user type, need to confirm what details will be asked for on the form. 
